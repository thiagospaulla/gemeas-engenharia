-- =====================================================
-- SQL PARA EXECUTAR DIRETO NO CONSOLE DO NEON
...
-- Gerar hash da senha (bcrypt de 'admin123'):
...
SELECT 
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'budgets') as budgets_criado,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'budget_items') as budget_items_criado,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'invoices') as invoices_criado,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'appointments') as appointments_criado,
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'active') as campo_active_criado,
    (SELECT COUNT(*) FROM users WHERE email = 'admin@gemeas.com.br') as admin_criado;

-- Se todos retornarem 1, está tudo OK! ✅

-- Verificar admin criado:
SELECT id, name, email, role, active FROM users WHERE email = 'admin@gemeas.com.br';
